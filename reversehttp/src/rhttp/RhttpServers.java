...
public class RhttpServers implements HttpProcessor {
	...
	public RhttpServers(HttpInputStream in, SocketList socketList)
			throws IOException {
	...
	void listServers(HttpOutputStream out) throws IOException {
		...
		if (socketList.isEmpty())
			...
			while (slEnum.hasMoreElements()) {
				...
				out.write("<hr>");
				out.write("For RHA " + aID
						+ ", we have allocated a tunneling proxy at ");
				...
				out.write("<a href=http://" + localHost + ":"
						+ localSS.getLocalPort() + "/>");
				out.write("http://" + localHost + ":" + localSS.getLocalPort()
						+ "/</a><br>");
				out.write("You need change your proxy configuration to use this proxy.<p>");
				// write direction to get to a PAC file
				out.write("Use this PAC file: <a href=http://" + localHost
						+ "/pac/" + aID + ".pac" + ">");
				out.write("http://" + localHost + "/pac/" + aID + " .pac"
						+ "</a><p>");
				out.write("Then browse: <a href=http://" + aID + ">http://"
						+ aID + "</a><p>\n");
				out.write("Kill connection: <a href=http://" + localHost
						+ "/close/" + aID + ">");
