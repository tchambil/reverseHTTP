...
public Proxy(ServerSocket ss, Socket server_socket, SocketList browserHash, boolean persistent) {
	t = new Thread(this, "Proxy Thread"); 
	System.out.println("Child thread: " + t);
	this.ss = ss;
	isServerSocket = true;
...
public void run() {
	...
	e.printStackTrace ();
	dataSent=false;
	System.out.println(t +" of preprocessType: "+ preprocessType +" has returned with exception.");
	return;
	}
	}
	try {
...
*/  
void createStreams(Socket client_socket, Socket server_socket) throws IOException {
// create the streams
try {
...
//http_from_client = new LoggedInputStream(from_client, HTTP.LOG_BIN+"RHAin.txt");
//else
//http_from_client = new LoggedInputStream(from_client, HTTP.LOG_BIN+ "RHPin.txt");
...
// TEST code
//System.out.printin(http-from-client.bytesAvailable() + " bytes availble to be read."); 200
...
throw new IOException("BID is null. Firewall sending error message?");
}
...
System.out.println("IOException in proxying. Socket seems dead.");
...
System.out.println("IOException in proxying. Socket to WWW server seems dead.");
//e.printStackTrace(;
throw e;
}
}
...
/*
* Sets the preprocess type which affects what kind of preprocessing needs to be done. 290*/
...
/*
* Sets the contentLength of the body. This forces the class to only proxy this many
* bytes of data.*/
...
this.contentLength = size;
 ...
 /*
* Gets the contentLength of the body. This forces the class to only proxy this many
* bytes of data.*/
