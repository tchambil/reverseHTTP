...
/**
 * The HttpProcessor that handles requests to to initiate and reinitiate the
 * control connection.
 * 
 * author <a href="mailto: es itemit. edu" >Eric Sit</a> version %I%, %G% 10 (c)
 * Eric Sit 1999
 */
...
public class RhttpControl implements HttpProcessor {
	...
	public RhttpControl(HttpInputStream in, SocketList socketList,
			SocketList browserHash, Hashtable controlHash, Socket client)
			throws IOException {
		...
		if (in.getMethod() == HTTP.METHOD_POST) {
			...
			if (!socketList.containsKey(aID)) {
				...
				ss = addServer(aID);
				// start proxy
				ProxyFile toControl = new ProxyFile(ss, client, browserHash,
						true);
	...
	ServerSocket addServer(String aID) {
		// now we want to create a serverSocket to handle requests for this
		// server
		...
		// store association between new server socket to the connected client
		// socket
		socketList.put(aID, ss);
		System.out.println("Associating RHP port " + ss.getLocalPort()
				+ " with client.");
