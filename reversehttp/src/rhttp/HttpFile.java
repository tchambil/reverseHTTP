...
public class HttpFile implements HttpProcessor {
	 ...
	 * 
	 * Client's file request for a file is passed into the contructor which
	 * throws any excpetions that result from the client's request. The
	 * constructor fully parses the client's request. If the client attempts to
	 * post 20 to a file, we throw an appropriate HttpException.
	 * 
	 * We translate the request into a file within the local HTML document
	 * directory, appending index.html if the request ends in /.
	 */

	public HttpFile(HttpInputStream in) throws IOException {
		if (in.getMethod() == HTTP.METHOD_POST)
			throw new HttpException(HTTP.STATUS_NOT_ALLOWED, "<TT>"
					+ in.getMethod() + " " + in.getPath() + "</TT>");
		...
		if (!file.exists())
			throw new HttpException(HTTP.STATUS_NOT_FOUND, "File <TT>"
					+ in.getPath() + "</TT> not found.");
		if (file.isDirectory())
	...
	/*
	 * When it is time to send a response, this method is called. We set
	 * Content-type and Content-length headers, then call sendHeaders() to send
