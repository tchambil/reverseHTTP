...
boolean init = true;
/*
* Creates an reverse HTTP agent object.
* The agent is for web host wwwHost, port wunwPort.
* The Reverse HTTP proxy to be given access is RHPHost, port RHPPort.*/
...
/**
* Starts the Reverse HITTP Agent.
* The agent is for web host wwwHost, port wwwPort.
* The Reverse HTTP proxy to be given access is pHost, port pPort.
* The firewall proxy is at proxyllost, port proryPort.
*Usage: lava RHA ccwwwlost> <wwwPort> <pilost> 'ccpPort> <proxyllost> <proxyPort>"
*/
...
System.out.println("Starting RHA for web server "+wwwHost+": "+wwwPort+  
", using Proxy "+ System.getProperties().getProperty("proxyHost") +
":" + System.getProperties().getProperty("proxyPort"));
RHA thisRHA = new RHA(wwwHost, wwwPort,RHPHost, RHPPort);
} catch (Exception e) {
System.err.println(e);
System.err.println("Usage: java RHA <wwwHost> <wwwPort><pHost> <pPort> <proxyHost> <proxyPort>");
}  
}
/**
...
* */
	...
	waitForData(httpFromControl);
		...
		if (init) {
			...
			{
			 System.err.println("Couldn't read initial headers on response");
			 e.printStackTrace();
			try {
	...
	System.out.println("Trying to encode as request and send back to RHP.");
	sendResponse();
...
try {
	control_socket = new Socket(System.getProperties().getProperty("proxyHo st"),
	...
	// TEST CODE
	///httpFromControl = new LoggedInputStream(from_control, "RHAin.txt");
	//
...
aID = wwwHost;
try {
	httpToControl.write(HTTP.METHOD_POST+ " http://"
	+ RHPHost +":"+ RHPPort +"/" +HTTP.CONTROL_BIN+" HTTP/1.0\r\n");
...
data_socket = new Socket (System.getProperties().getProperty("proxyHo st"),
Integer.parseInt(System.getProperties().getProperty("proxyPort")));
} catch (UnknownHostException e) {
System.err.println("Don't know about host: RHP.");
System.exit(1);
} catch (IOException e) {
System.err.println("Couldn't get I/0 for the connection to: RHP. RHP seems to have disappeared.");
System.exit(1);  
}
