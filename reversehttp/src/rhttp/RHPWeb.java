 ...
 * 
 * 53 60 70 80 90 author <a href= "mailto: esitemit.edu" >Eric Sit</a> 10 "
 * version %I%, %G% (c) Eric Sit 1999
 */
...
public class RHPWeb extends Thread {
	...
	public RHPWeb(ServerSocket socket, SocketList socketList,
			SocketList browserHash) {
	...
	public void run() {
		try {
			for (;;) {
				...
				// Wait for client to connect. The method will block, and when
				// it
	...
	protected HttpProcessor getProcessor(HttpInputStream httpIn) {
		try {
			...
			if (httpIn.getPath().startsWith(HTTP.SERVERS_BIN))
				...
				return new RhttpControl(httpIn, socketList, browserHash,
						controlHash, client);
			...
			return new HttpException(HTTP.STATUS_INTERNAL_ERROR, "<PRE>"
					+ trace + "</PRE>");
		}
	}
}
