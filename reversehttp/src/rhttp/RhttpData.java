...
public class RhttpData implements HttpProcessor {
	...
	// we are guarenteed that client is distinct because we just gota new socket
	// from RHPWeb's accept call
	public RhttpData(HttpInputStream in, SocketList socketList,
		...
		if (in.getMethod() == HTTP.METHOD_POST) {
			...
			// bID);
			// using the browser ID coming in, try to match it up with the
			// original browser socket
	...
	public void processRequest(HttpOutputStream out) throws IOException {
		...
		toBrowser.setContentLength(Integer.parseInt(in
		...
		// get rid of the browser socket entry in the hash table since we don't
		// need it anymore.
		...
		if (!toBrowser.dataSent) {
			...
			// A possible reason for this is that the browser has already closed
			// the socket.
			// For example, the user clicked on another link before it received
			...
			http_to_server.write("HTTP/1.0 " + HTTP.STATUS_INTERNAL_ERROR
					+ " Ok\r\n");
			...
			// The plan is to ignore this particular bit of data coming in along
			// the data connection.
			// We'll just wait till another request comes along the control
			// connection.
		} else {
			// success
			http_to_server.write("HTTP/1.0 " + HTTP.STATUS_NO_CONTENT
					+ " Ok\r\n");
			http_to_server.write("\r\n");
			http_to_server.flush();
		}
