...
public class ProxyFile extends Thread {
	...
	File file;
	/*
	 * The contentLength of the HTTP body. 64
	 */
	...
	public boolean dataSent = false;/*
									 * If true, the ProxyFile thread will return
									 * at the first available moment. This
									 * usually right after the serversocket
									 * accepts.
									 */
	...
	boolean persistent = false; /*
								 * Creates a proxy to copy data from the socket
								 * to outSocket. If persistent, connection to
								 * the socket will remain open after proxying
								 * data.
								 */
	 ...
	 * BrowserHash maintains a hash between the browserID and the accepted
	 * socket. If persistent, connection to the socket will remain open after
	 * proxying data. 65
	 */

	public ProxyFile(ServerSocket ss, Socket outSocket, SocketList browserHash,
			boolean persistent) {
		t = new Thread(this, "ProxyFile Thread");
		System.out.println("Child thread: " + t);
		this.ss = ss;
	 ...
	 * 100 Main execution of Proxy thread.
	 */
		...
		while (loop) {
			...
			try {
				synchronized (outSocket) {
					...
					dataSent = true;
					System.out
							.println("Proxyfile has successfully proxied along the data!");
				...
				if (getPreprocess().equals("encodeAsResponse")) {
					...
					try {
						System.out
								.println("Trying to close control connection.");
						outSocket.close();
					} catch (IOException f) {
					...
					dataSent = false;
					// we should wait until another control connection is
					// established.
				}
			} finally {
	...
	void getClient() throws IOException {
		...
		try {
			...
			// generate socket key.
			bID = s.getInetAddress().getHostAddress() + " "
	...
	void toFile() throws IOException {
		...
		String filename = HTTP.TMP_BIN
				+ new Integer(s.getLocalPort()).toString() + ".tmp";
		...
		try {
			...
			System.err
					.println("I0 Exception in creating fileoutputstream for writing");
			fout = null;
			e.printStackTrace();
			throw e;
		}
		...
		if (preprocessType.equals(" encodeAsResponse")) {
			...
			String requestString = http_from.getMethod() + "1 "
					+ http_from.getPath() + " " + "HTTP/"
					+ String.valueOf(http_from.getVersion()) + "\r\n";
			try {
			...
			while (headerNames.hasMoreElements()) {
				...
				try {
					fout.write(stringToBytes(header + ": "
							+ http_from.getHeader(header) + "\r\n"));
				} catch (IOException e) {
					System.err
							.println("Could not write the rest of the headers to disk");
					e.printStackTrace();
					throw e;
				}
			}
			...
			if (http_from.getMethod() == HTTP.METHOD_POST) {
				...
				try {
					while ((bytes_read = http_from.read(buffer)) != -1) {
						...
						if ((bytes_read_total >= contentLength)
								&& (contentLength != -1))
							break;
					}
				} catch (IOException e) {
			...
			try {
				...
				System.err
						.println("I0Exception in write (part of encodeAsRequest)");
				e.printStackTrace();
				throw e;
			} finally {
				if (fout != null)
	...
	void createStreams() throws IOException {
		try {
			...
			// http_to_server = new LoggedOutputStream(to_server,
			// http_from_server, HTTP.LOG_BIN+"RHPout.txt");
			// else
			// http_to_server = new LoggedOutputStream(to_server,
			// http_from_server, HTTP.LOG_BIN+"RHAout.txt");
		} catch (IOException e) {
	...
	void preProcess() throws IOException {
		if (preprocessType.equals("encodeAsResponse")) {
			...
			http_to_server.setHeader("Content-Length",
			...
			// send
			try {
				http_to_server.write(HTTP.METHOD_POST + " http://"
						+ RHA.getRHPHost() + ":" + RHA.getRHPPort() + "/"
						+ HTTP.DATA_BIN + " HTTP/1.0" + "\r\n");
				...
				http_to_server.write("Content-length: "
						+ String.valueOf(contentLength) + " \r\n");
	...
	void toSocket() throws IOException {
		...
		try {
			...
			System.err
					.println("We couldn't write to the socket going to server for some reason!");
			try {
				from_file.close();
			...
			throw new IOException(
					e.toString()
							+ "Couldn't write to socket going to server for some reason!");
		} finally {
			try {
				from_file.close();
			} catch (IOException e) {
	 ...
	 * Changes the outSocket to a new socket. Useful when a control connection
	 * needs to be changed to a new socket. 510
	 */
	 ...
	 * Reads a line of text from an inputstream. 530
	 */
